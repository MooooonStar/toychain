...
一旦某个矿工找到了满足条件的Nonce值，便可以将区块头和交易列表打包成区块，添加到区块链里去。而打包这个区块的矿工,就获得了创币的特权，并且得到了交易中包含的手续费。所以我们看到挖矿实际上是通过工作量证明将交易打包成区块，并链接到链上使得交易得到确认的过程，是BTC里不可缺少的一部分。没有了挖矿，所有的交易都只会留着节点的内存里。
